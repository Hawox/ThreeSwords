 ...
 *
 * EDIT Hawox: I might have edited it slightly from the original
 */
...
public class Sprite implements Cloneable, Serializable {
    ...
    /*
     * TODO: HAWOX EDIT
     * My one sprite per object item might be comming from here lets see if this fixes it
     * EDIT 2: yeah that's negitive
     *
    static public boolean collidesWith(Sprite spriteA, Sprite spriteB, boolean pixelPerfect) {
        boolean isColliding=false;

        Rectangle r1 = getBounds(spriteA);
        Rectangle r2 = getBounds(spriteB);
        
        if(r1.intersects(r2)) {
            if(pixelPerfect) {
                //isColliding = pixelPerfectCollision(spriteB, r1, r2);
            }else {
                isColliding = true;
            }
        }
