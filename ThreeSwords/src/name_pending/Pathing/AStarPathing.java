...
//TODO Diagnoal distance h(n) = D * max(abs(n.x-goal.x), abs(n.y-goal.y))
...
public class AStarPathing implements Runnable{
		...
		do{
			...
			if(waitingList.size() > 0)
			{
				...
				if(pd.pathable instanceof Entity)
				{
					...
					theGame.getConsole().addText("Found a path in " + Long.toString(System.currentTimeMillis()-start) + " ms for entity named: " + e.getName());
				}
			}
	...
	class AStarSearch extends AStar<Point>
	{
		...
		/*
		 * TODO Currently this assumes there are no obstacles in the way
		...
		protected Double g(Point from, Point to) {
			...
			/* TODO not using this yet, will if I see a need to later
		...
		protected List<Point> generateSuccessors(Point node) {
			...
			//TODO might be a better way to do this but this should work for now
